...
if($table == "vehicule"){
    ...
    if(isset($_POST['send'])){
        ...
        if($nbrError===0){
            updateVehicule($_POST['marque'],$_POST['modele'],$_POST['couleur'],$_POST['immatriculation'],$id); 
        }    
}
        ...
        <div class="formulaireSide">
            ...
            <form action="" method="post" class="formu">
                <div class="form-floating">
                    <input type="text" value ="<?php echo $vehicule['marque'] ?>" class="form-control" placeholder="marque" name="marque" required="required">
                    <label for="floatingInput">Marque</label>
                    <?php erreur($errorMarque) ?>
                </div>
                <?php simpleBr() ?>
                <div class="form-floating">
                    <input type="text" value ="<?php echo $vehicule['modele'] ?>" class="form-control" placeholder="modele" name="modele" required="required">
                    <label for="floatingInput">Modele</label>
                    <?php erreur($errorModele) ?>
                </div>
                <?php simpleBr() ?>
                <div class="form-floating">
                    <input type="text" value ="<?php echo $vehicule['couleur'] ?>" class="form-control" placeholder="couleur" name="couleur" required="required">
                    <label for="floatingInput">Couleur</label>
                    <?php erreur($errorCouleur) ?>
                </div>
                <?php simpleBr() ?>
                <div class="form-floating">
                    <input type="text" value ="<?php echo $vehicule['immatriculation'] ?>" class="form-control" placeholder="immatriculation" name="immatriculation" required="required">
